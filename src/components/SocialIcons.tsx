...
const SocialIcons = () => {
  const socialLinks = [
    {
      ...
      color: 'bg-[#1db954]',
      icon: (
        <svg version="1.1" viewBox="0 0 100 100" className="w-6 h-6">
          <path
  ...
  return (
    <ul className="flex flex-col justify-center gap-2.5 items-center">
      {socialLinks.map((social) => (
        <li key={social.name} className="relative mx-2.5">
          <div
            className={`absolute -top-2.5 left-1/2 transform translate-x-1/2 text-white px-2.5 py-1.5 rounded-2xl opacity-0 invisible text-sm transition-all duration-300 ease-in-out pointer-events-none ${social.color}`}
          >
            {social.name}
          </div>
          <a
            ...
            className="relative overflow-hidden flex justify-center items-center w-12 h-12 rounded-[20%] text-gray-600 bg-white transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-black/50 hover:text-white group"
          >
            <div
              className={`absolute top-auto bottom-0 left-0 w-full h-0 transition-all duration-300 ease-in-out ${social.color} group-hover:h-full`}
            ></div>
            {social.icon}
          </a>
        </li>
